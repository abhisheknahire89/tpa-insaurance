...
export const AdmissionCostStep: React.FC<AdmissionCostStepProps> = ({
    ...
    return (
        <div className="space-y-5">
            <h2 className="text-base font-semibold text-white">Admission & Cost Estimation</h2>

            {/* Admission Details */}
            <div className="bg-gray-900/30 border border-white/5 rounded-2xl p-5 space-y-4 shadow-sm">
                <h3 className="font-semibold text-blue-400 text-xs flex items-center gap-2 uppercase tracking-wider">🏥 Admission Details</h3>
                <div className="flex gap-4">
                    {['Emergency', 'Planned'].map(type => (
                        <label key={type} className="flex items-center gap-2.5 cursor-pointer bg-gray-950/40 border border-white/5 hover:border-white/10 rounded-xl px-4 py-2.5 text-xs text-gray-300 transition-colors select-none">
                            <input type="radio" name="admType" value={type}
                                checked={admission.admissionType === type}
                                onChange={() => updateField({ admissionType: type as any })} className="accent-blue-500 w-4 h-4" />
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs text-gray-400 font-semibold mb-1.5">Date of Admission <span className="text-red-400 font-bold ml-0.5">*</span></label>
                    ...
                    <div>
                        <label className="block text-xs text-gray-400 font-semibold mb-1.5">Time of Admission</label>
                        <input type="time" value={admission.timeOfAdmission ?? ''} onChange={e => updateField({ timeOfAdmission: e.target.value })}
                            className="w-full bg-gray-900 border border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-colors" />
                    </div>
                </div>
                <div>
                    <label className="block text-xs text-gray-400 font-semibold mb-2">Room Category</label>
                    <div className="flex flex-wrap gap-2">
                        {ROOM_CATEGORIES.map(cat => (
                            <button key={cat} onClick={() => handleRoomCategory(cat)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${admission.roomCategory === cat ? 'bg-blue-600 border-blue-500 text-white shadow-sm shadow-blue-500/10' : 'bg-gray-900 border-white/10 text-gray-400 hover:text-white'}`}
                                type="button">
                                {cat}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div>
                        <label className="block text-xs text-gray-400 font-semibold mb-1.5">Ward Days</label>
                        <input type="number" value={admission.expectedDaysInRoom ?? ''} onChange={e => { updateField({ expectedDaysInRoom: +e.target.value, expectedLengthOfStay: (+e.target.value) + (admission.expectedDaysInICU ?? 0) }); updateCost({ expectedRoomDays: +e.target.value }); }}
                            className="w-full bg-gray-900 border border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500/50 placeholder-gray-600 transition-colors" min={0} />
                    </div>
                    <div>
                        <label className="block text-xs text-gray-400 font-semibold mb-1.5">ICU Days</label>
                    ...
                    <div>
                        <label className="block text-xs text-gray-400 font-semibold mb-1.5">Total Length of Stay</label>
                        <input readOnly value={`${(admission.expectedLengthOfStay ?? 0)} days`}
                            className="w-full bg-gray-900/60 border border-white/5 rounded-xl px-3.5 py-2.5 text-sm text-gray-500 select-none outline-none" />
                    </div>
                </div>
            </div>
            ...
            {matchedPackage && (
                <div className="bg-gradient-to-r from-emerald-950/20 via-teal-950/10 to-transparent border border-emerald-500/20 rounded-2xl p-5 space-y-3.5 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="mt-3.5 bg-black/30 border border-white/5 rounded-xl p-4">
                            <div className="flex justify-between items-center mb-3">
                                <div>
                                    <div className="text-white text-sm font-semibold">{matchedPackage.package_name}</div>
                                    <div className="text-emerald-400 text-xs font-mono font-bold mt-0.5">{matchedPackage.hbp_code}</div>
                                </div>
                                <div className="text-right">
                                    <div className="text-emerald-400 font-bold text-lg">{formatCostDisplay(matchedPackage.package_rate_inr)}</div>
                                    <div className="text-gray-500 text-[10px] font-bold uppercase tracking-wider mt-0.5">Govt Rate</div>
                                </div>
                            </div>
                            ...
                            {matchedPackage.private_tpa_rates && (
                                <div className="mt-3.5 pt-3.5 border-t border-white/5">
                                    <div className="text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Typical Private TPA Packages</div>
                                    <div className="grid grid-cols-3 gap-2">
                                        {Object.entries(matchedPackage.private_tpa_rates as Record<string, { min: number, max: number }>).map(([tpa, limits]) => (
                                            <div key={tpa} className="bg-white/5 rounded-xl px-3 py-2 border border-white/5 text-center">
                                                <div className="text-[10px] text-gray-400 font-medium capitalize">{tpa.replace('_', ' ')}</div>
                                                <div className="text-xs text-blue-400 font-bold mt-0.5">₹{(limits.min / 1000).toFixed(0)}k - {(limits.max / 1000).toFixed(0)}k</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="mt-4 flex gap-2">
                                <button
                                    ...
                                    className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all duration-200 ${cost.isPackageRate && cost.packageCode === matchedPackage.hbp_code ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-500/10' : 'bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/10'}`}
                                    type="button"
                                >
                                ...
                                {matchedPackage.private_tpa_rates?.medi_assist && (
                                    <button
                                        ...
                                        className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all duration-200 ${cost.isPackageRate && cost.packageAmount === matchedPackage.private_tpa_rates.medi_assist.min ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/10' : 'bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/10'}`}
                                        type="button"
                                    >
                                        Apply TPA Base Rate
                                    </button>
                                ...
                                {cost.isPackageRate && (
                                    <button onClick={clearPackage} className="px-3 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs rounded-xl font-bold border border-white/5 transition-colors" type="button">
                                        Clear
                                    </button>
                                )}
                            </div>
            ...
            <div className="bg-gray-900/30 border border-white/5 rounded-2xl p-5 space-y-4 shadow-sm">
                <h3 className="font-semibold text-blue-400 text-xs flex items-center gap-2 uppercase tracking-wider">📋 Past Medical History</h3>
                <div className="grid grid-cols-2 gap-3">
                    {PAST_CONDITIONS.map(([key, label]) => (
                        <div key={key} className="flex items-center gap-3 bg-gray-950/40 border border-white/5 hover:border-white/10 rounded-xl px-4 py-3 text-xs text-gray-300 transition-colors select-none">
                            <input type="checkbox"
                            ...
                            {pmh[key]?.present && (
                                <input value={pmh[key]?.duration ?? ''} placeholder="Duration..."
                                    ...
                                    className="w-24 bg-gray-900 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none placeholder-gray-600 transition-colors" />
                            )}
                        </div>
                    ))}
                </div>
                <div className="pt-2 border-t border-white/5">
                    <label className="flex items-center gap-2.5 cursor-pointer bg-gray-950/40 border border-white/5 hover:border-white/10 rounded-xl px-4 py-3 text-xs text-gray-300 transition-colors select-none">
                        <input type="checkbox" checked={admission.previousHospitalization?.wasHospitalizedBefore ?? false}
                            onChange={e => updateField({ previousHospitalization: { wasHospitalizedBefore: e.target.checked } })} className="accent-blue-500 w-4 h-4 rounded" />
                    ...
                    {admission.previousHospitalization?.wasHospitalizedBefore && (
                        <div className="grid grid-cols-2 gap-4 mt-3">
                            <input value={admission.previousHospitalization?.details ?? ''} placeholder="Hospital/Diagnosis details..."
                                ...
                                className="bg-gray-900 border border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none placeholder-gray-600" />
                            <input type="date" value={admission.previousHospitalization?.dateOfLastHospitalization ?? ''}
                                ...
                                className="bg-gray-900 border border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none" />
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="bg-gray-900/30 border border-white/5 rounded-2xl p-5 space-y-4 shadow-sm">
                <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-blue-400 text-xs flex items-center gap-2 uppercase tracking-wider">💰 Estimated Cost Break-up</h3>
                    <div className="text-xs text-gray-500 flex items-center gap-2">
                        {cost.isPackageRate ? (
                            <span className="text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-md border border-emerald-500/20 text-[10px] font-bold uppercase tracking-wider">Package Rate Applied</span>
                        ) : (
                            <span className="text-[10px] text-gray-400 italic">Defaults from rate card — adjust as needed</span>
                        )}
                    </div>
                </div>
                ...
                {cost.isPackageRate ? (
                    <div className="bg-black/35 rounded-xl p-4 border border-emerald-500/10">
                        <div className="flex justify-between items-center">
                            <div>
                                <div className="text-sm font-semibold text-emerald-300">{cost.packageName || 'Procedure Package'}</div>
                                {cost.packageCode && <div className="text-[10px] text-gray-500 font-mono font-bold mt-1">{cost.packageCode}</div>}
                            </div>
                            <div className="text-right">
                                <label className="block text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Package Amount</label>
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        className="bg-gray-900 border border-emerald-500/30 text-emerald-400 font-bold rounded-lg px-3 py-1.5 text-right w-32 focus:outline-none focus:border-emerald-400 transition-colors"
                                    />
                                </div>
                            </div>
                        </div>

                        <div className="text-[11px] text-emerald-500/75 mt-3.5 flex items-start gap-1.5 leading-normal">
                    ...
                    <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
                        <table className="w-full text-xs text-left">
                            <thead>
                                <tr className="text-[10px] text-gray-500 uppercase tracking-wider bg-gray-900/40 border-b border-white/5">
                                    <th className="px-4 py-3 font-bold">Billing Head</th>
                                    <th className="px-4 py-3 text-right font-bold w-32">Rate Details</th>
                                    <th className="px-4 py-3 text-right font-bold w-32">Amount (₹)</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-white/5 font-medium">
                                <tr>
                                    <td className="px-4 py-3 text-gray-300">Room Rent (Daily)</td>
                                    <td className="px-4 py-3 text-right text-gray-500">₹{cost.roomRentPerDay ?? 0} × {cost.expectedRoomDays ?? 0} days</td>
                                    <td className="px-4 py-3 text-right text-white font-mono">{(cost.roomRentPerDay ?? 0) * (cost.expectedRoomDays ?? 0)}</td>
                                </tr>
                                <tr>
                                    <td className="px-4 py-3 text-gray-300">Nursing Charges (Daily)</td>
                                    <td className="px-4 py-3 text-right text-gray-500">₹{cost.nursingChargesPerDay ?? 0} × {cost.expectedRoomDays ?? 0} days</td>
                                    <td className="px-4 py-3 text-right text-white font-mono">{(cost.nursingChargesPerDay ?? 0) * (cost.expectedRoomDays ?? 0)}</td>
                                </tr>
                                <tr>
                                    <td className="px-4 py-3 text-gray-300">ICU Charges (Daily)</td>
                                    <td className="px-4 py-3 text-right text-gray-500">₹{cost.icuChargesPerDay ?? 0} × {cost.expectedIcuDays ?? 0} days</td>
                                    <td className="px-4 py-3 text-right text-white font-mono">{(cost.icuChargesPerDay ?? 0) * (cost.expectedIcuDays ?? 0)}</td>
                                </tr>
                                {[
                                    ...
                                    <tr key={key}>
                                        ...
                                        <td className="px-4 py-2.5 text-right text-gray-500 italic text-[10px]">manual override</td>
                                        <td className="px-4 py-2.5 text-right">
                                            <input type="number" value={(cost as any)[key] ?? 0} onChange={e => updateCost({ [key]: +e.target.value } as any)}
                                                className="w-28 bg-gray-900 border border-white/10 rounded-lg px-2.5 py-1 text-xs text-right text-white font-mono focus:outline-none focus:border-blue-500/50 transition-colors" />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                {/* Totals */}
                <div className={`rounded-xl p-4.5 border transition-all duration-300 ${totals.exceedsSumInsured ? 'bg-red-900/10 border-red-500/20' : 'bg-emerald-950/5 border-emerald-500/10'}`}>
                    <div className="flex justify-between items-center text-sm font-bold">
                        <span className="text-gray-300">Total Estimated Cost</span>
                        <span className="text-white text-base font-mono">{formatCostDisplay(totals.totalEstimatedCost)}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs mt-3.5">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <input type="number" value={cost.amountClaimedFromInsurer ?? totals.totalEstimatedCost}
                                ...
                                className="w-32 bg-gray-900 border border-white/10 rounded-lg px-2.5 py-1 text-xs text-right text-white font-mono focus:outline-none focus:border-blue-500/50 transition-colors" />
                        </div>
                    </div>
                    <div className="flex justify-between items-center text-xs mt-3.5 border-t border-white/5 pt-3">
                    ...
                    {totals.exceedsSumInsured && (
                        <div className="mt-3.5 bg-red-500/5 border border-red-500/15 rounded-xl p-3 text-red-300 text-xs font-semibold leading-relaxed">
                    ...
                    {!totals.exceedsSumInsured && sumInsured > 0 && (
                        <div className="mt-3.5 text-emerald-400 text-xs font-bold flex items-center gap-1.5">
                            <span>✓</span>
